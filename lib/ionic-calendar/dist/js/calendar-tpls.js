...
angular.module("templates/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("templates/rcalendar/month.html",
        "<div>\n" +
        "    <ion-slide-box class=\"monthview-slide light-bg\" on-slide-changed=\"slideChanged($index)\" does-continue=\"true\"\n" +
        ...
        // "        <table class=\"table table-bordered table-striped table-fixed event-detail-table light-bg\">\n" +
        // "            <tr  class=\" light-bg\" ng-repeat=\"event in selectedDate.events\" ng-click=\"eventSelected({event:event})\">\n" +
        ...
        // "                <td class=\"event-detail item item-icon-right item-icon-left padding-10\"><div ><i class='ion ion-grid dark'></i><h3>{{::event.title}}</h3><p>{{::event.from_date}}</p><p>{{::event.text}}</p><i class='ion ion-ios-bell theme_orange'></i></div></td>\n" +
        // "            </tr>\n" +
        ...
        "<div class='list' ><div class='item item-icon-right item-icon-left padding-10' ng-repeat=\"event in selectedDate.events\" ng-click=\"eventSelected({event:event})\"><i class='icon ion ion-grid dark'></i><h2 style='margin-left:40px;'>{{::event.title}}</h2><p style='margin-left:40px;'>{{::event.text}}</p><p style='position: absolute;right: 40px;color: #3a87ad;top: 10px;font-size: 12px;font-weight: bold;'>{{::event.data.from_time|date: 'HH:mm' +' to '}}{{event.data.to_time|date: 'HH:mm'}}</p><i class='icon ion ion-ios-bell theme_orange'></i></div></div>" +
        "    </ion-content>\n" +
        "</div>\n" +
        "");
}]);
