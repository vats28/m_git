angular.module('starter.addLoved', [])

    .controller('addLovedCtrl', function($scope, generic_http_post_service, date_picker) {
        $scope.temp ={};
        $scope.temp.access ={};
        ...
        $scope.addLovedOnes = function () {
            try {
              ...
              //  alert(JSON.stringify($scope.requestData));
                generic_http_post_service.getDetails(generic_http_post_service.getServices().REGISTER_LOVEDONE,
                    $scope.requestData, $scope.addLovedOnes_callback);
            }catch(error){
                alert(error);
                $scope.hideLoader();
            }
